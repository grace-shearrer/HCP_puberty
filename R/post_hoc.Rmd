...
NM0<-cbind(subs,netmat)
summary(NM0$V35)
names(NM0)
#NMm<-cbind(subs,netmat)
...
d40<-subset(d30, d30$BMI_C != 'NA')
summary(d4)
summary(is.na(d40$V36))
d40<-d40[!is.na(d40$V36),]
dim(d4)
...
netmat_m<-read.table("~/Google Drive/HCP_graph/1200/5000perms/R2R/hmotion_corr/netmats1.csv", header=F, sep=",")
summary(netmat_m)
names(netmat_m)
colnames(netmat_m)[1]<-"Subject"
names(netmat_m)
library(dplyr)
ALLm<-netmat_m %>% 
    group_by(Subject) %>%
...
summary(ALLm$V35)
ALLm$Subject<-as.factor(ALLm$Subject)

d3m<-join(d2,ALLm)
summary(d3m)

d4m<-subset(d3m, d3m$BMI_C != 'NA')
summary(d4m)
...
p1<-ggplot(d4, aes(Menstrual_AgeBegan, V181)) +
  geom_point(shape=1) + 
...
n=50
p1<-ggplot(subset(d4, !is.na(d4$AoM)), aes(BMI, V37, group=AoM, color=AoM)) +
...
p2<-ggplot(subset(d4, !is.na(d4$AoM)), aes(BMI, V179, group=AoM, color=AoM)) +
  geom_point(aes(shape=AoM), size=2) + 
  geom_smooth(method=lm, aes(linetype=AoM))+
  scale_y_continuous(name="Correlation between\n IC12 and IC13")+
  scale_x_continuous(name="BMI")+
...
p3<-ggplot(subset(d4, !is.na(d4$AoM)), aes(BMI, V19, group=AoM, color=AoM)) +
  geom_point(aes(shape=AoM), size=2) + 
  geom_smooth(method=lm, aes(linetype=AoM))+
  scale_y_continuous(name="Correlation between\n IC2 and IC3")+
  scale_x_continuous(name="BMI")+
...
myvars<-c("Family_ID","Subject","Menstrual_AgeBegan","BMI","ZygosityGT","Age_in_Yrs","Race","Gender","V37","V79","V179","V193")
d5<-twins[myvars]
...
d6 <- fast.reshape(d5, id="Family_ID",varying=c("BMI","Menstrual_AgeBegan","Subject","V37","V79","V179","V193"))
d6<-na.omit(d6)
head(d6)
```
...
mz_V178<-subset(d6, ZygosityGT ==  "MZ"  )[,c("V1791","V1792")]
p_V178_mz<-scatterdens(mz_V178)

mz_V36<-subset(d6, ZygosityGT ==  "MZ"  )[,c("V371","V372")]
...
dz_V178<-subset(d6, ZygosityGT ==  "DZ"  )[,c("V1791","V1792")]
p_V178_dz<-scatterdens(dz_V178)

dz_V36<-subset(d6, ZygosityGT ==  "DZ"  )[,c("V371","V372")]
...
```{r}
l2.0<-lm(V179 ~ 1+BMI*Menstrual_AgeBegan+Age_in_Yrs+Race, data=d4)

summary(l2.0)
l2 <- twinlm(V179 ~ 1+BMI*Menstrual_AgeBegan+Age_in_Yrs+Race, data=d5,
...
```{r}
l3.0<-lm(V37 ~ BMI*Menstrual_AgeBegan+Age_in_Yrs+Race, data=d4)
summary(l3.0)
l3 <- twinlm(V36 ~ 1+BMI*Menstrual_AgeBegan+Age_in_Yrs+Race, data=d50,
DZ= "DZ"  , zyg=  "ZygosityGT"  , id="Family_ID", type="aced", missing = T)  
summary(l3)
# AIC
AIC(l3.0)
