...
# Edge analysis
```{r}
early13<-read.table("~/Google Drive/HCP_graph/1200/datasets/edges_early_IC13.csv", sep=",", header=T)
head(early13)
late13<-read.table("~/Google Drive/HCP_graph/1200/datasets/edges_late_IC13.csv", sep=",", header=T)
head(late13)
IC13<-join(early13, late13)
summary(IC13)
```

```{r}
t0<-lm(weight~modules, data=late13)
summary(t0)
means.t0 <- lsmeans(t0, specs = "modules")
x<-pairs(means.t0, adjust = "bon")
library(data.table)
...
```{r}
head(IC13)
IC13_e<-subset(early13, early13$modules == "(2, 4)" | early13$modules=="(4, 2)")
IC13_l<-subset(late13, late13$modules == "(2, 4)" | late13$modules=="(4, 2)")
summary(IC13_e)
head(IC13_l)
t.test(IC13_e$weight,IC13_l$weight)
```
...
```{r}
# Basic violin plot
p <- ggplot(late13, aes(x=modules, y=weight, color=modules, fill=modules)) +
  geom_violin()+ theme_classic()
p
```
...
```{r}
test1<-subset(df, df$IC == 'IC13' & df$moduleX == '2' & df$moduleY == '4')
head(test1)
t.test(y~x)
t1<-t.test(test1$edge_weight~test1$early_late)
t1
```
