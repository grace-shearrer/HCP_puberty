...
netmat_m<-read.table("~/Google Drive/HCP_graph/1200/5000perms/R2R/hmotion_corr/netmats1.csv", header=T, sep=",")
names(netmat_m)<-names(NM)
motion<-read.table("~/Google Drive/HCP_graph/1200/datasets/HCP1200-Motion-Summaries.csv", header=T, sep=",")
head(motion)
motion$mean_motion = with(motion, ave(x = motionc, Subject, FUN = mean))
motion<-motion[!duplicated(motion$Subject), ]
head(motion)
motion<-motion[!duplicated(motion$Subject), ]

summary(netmat_m[,2:20])
netmat_m[2:226]<-as.numeric(netmat_m[2:226])

means = with(netmat_m, ave(x = netmat_m[,2:226], Subject, FUN = mean))
head(means)
summary(netmat_m[,2:20])
