...
subs4<-read.table("~/Google Drive/HCP_graph/1200/datasets/subs510.txt", header=F, sep="\t")

netmat<-read.table("~/Google Drive/HCP_graph/1200/datasets/3T_HCP1200_MSMAll_d15_ts2_nets/netmats1.txt", header=F, sep=" ")

NM<-read.table("~/Google Drive/HCP_graph/1200/5000perms/check/check5000perms/netmats1.txt", header=F, sep="\t")

NMm<-read.table("~/Google Drive/HCP_graph/1200/5000perms/R2R/hmotion_corr/netmats1.txt", header=F, sep="\t")
...
#### Original netmats
```{r}
NMorig<-cbind(subs,netmat)
summary(NMorig$V36)


d3orig<-join(d2,NMorig)

d3orig<-join(d2,NMorig)

d4orig<-subset(d3orig, d3orig$group != 'NA')
summary(is.na(d4orig$V36))
d4orig<-d4orig[!is.na(d4orig$V36),]
dim(d4orig)
```
#### New netmats
```{r}
colnames(subs4)[1]<-"Subject"
Subject<-subs4$Subject
NMnew<-cbind(Subject,NM)
...
d3new<-join(d2,NMnew)
dim(d3new)
d4new<-subset(d3new, d3new$group != 'NA')
dim(d4new)
summary(is.na(d4new$V36))
d4new<-d4new[!is.na(d4new$V36),]
dim(d4new)
d4<-d4new
```
#### Motion
```{r}
head(motion)
motion$mean_motion = with(motion, ave(x = motion, Subject, FUN = mean))
head(motion)
motion<-motion[!duplicated(motion$Subject), ]

colnames(subs4)[1]<-"Subject"
Subject<-subs4$Subject
NMm<-cbind(Subject,NMm)
names(NMm)
NMm<-NMm %>%
    group_by(Subject) %>%
    summarise_all("mean")
...
d3m<-join(d2,NMm)
d4m<-subset(d3m, d3m$group != 'NA')
dim(d4m)
summary(is.na(d4m$V36))
d4m<-d4m[!is.na(d4m$V36),]
dim(d4m)
d4m<-join(d4m,motion)
describe(d4m$BMI)
...
levels(d4m$Race)
d4m$Race<-factor(d4m$Race, levels = c("White","Black or African Am.","Asian/Nat. Hawaiian/Othr Pacific Is.","Am. Indian/Alaskan Nat.", "More than one","Unknown or Not Reported"))
levels(d4m$Race)
```

#### Repeated measures corr data
```{r}
early23<-read.table("~/Google Drive/HCP_graph/1200/datasets/early_IC_2_3.csv", sep=",", header=TRUE)
early36<-read.table("~/Google Drive/HCP_graph/1200/datasets/early_IC_3_6.csv", sep=",", header=TRUE)
early1213<-read.table("~/Google Drive/HCP_graph/1200/datasets/early_IC_12_13.csv", sep=",", header=TRUE)
```

```{r}
late23<-read.table("~/Google Drive/HCP_graph/1200/datasets/late_IC_2_3.csv", sep=",", header=TRUE)
late36<-read.table("~/Google Drive/HCP_graph/1200/datasets/late_IC_3_6.csv", sep=",", header=TRUE)
late1213<-read.table("~/Google Drive/HCP_graph/1200/datasets/late_IC_12_13.csv", sep=",", header=TRUE)
```



```{r}
sapply(early23, class)
drops <- c("high_low_2","high_low_3", "IC_3","IC_2")
early23<-early23[ , !(names(early23) %in% drops)]
late23<-late23[ , !(names(late23) %in% drops)]
n<-names(late23)

early23<-early23[ ,(names(early23) %in% n)]
late23<-late23[ ,(names(late23) %in% n)]
...
```{r}
x<-cor(l23, use="complete.obs")
x[abs(x) < 0.3] <- 0
dim(x)
x
y<-cor(e23, use="complete.obs")
dim(y)
y[abs(y) < 0.3] <- 0
y
cortest.mat(x,y,n1=125,n2 =87)
```
...
x
row.names(x)<-c("Intercept","Menstrual age began (yrs)","Age (yrs)","Native American" ,"Asian American, Native Hawaiian, Other Pacific Islander", "Black or African American", "More than one race","Unknown race or not reported")
x
```
...
# Depression 
```{r}
summary(d4$SSAGA_Depressive_Sx)
mDepression1<-lm(SSAGA_Depressive_Ep~BMI*Menstrual_AgeBegan+Race+Age_in_Yrs, data = d4)
summary(mDepression1)

summary(d4$SSAGA_Depressive_Ep)
mDepression2<-lm(SSAGA_Depressive_Sx~BMI*Menstrual_AgeBegan+Race+Age_in_Yrs, data = d4)
summary(mDepression2)

```

```{r}
summary(d4$NEOFAC_A)
mNEO1<-lm(NEOFAC_A~BMI*Menstrual_AgeBegan+Race+Age_in_Yrs, data = d4)
summary(mNEO1)

summary(d4$NEOFAC_O)
mNEO2<-lm(NEOFAC_O~BMI*Menstrual_AgeBegan+Race+Age_in_Yrs, data = d4)
summary(mNEO2) #significant but wouldn't survive correction for multiple comparisons

summary(d4$NEOFAC_C)
mNEO3<-lm(NEOFAC_C~BMI*Menstrual_AgeBegan+Race+Age_in_Yrs, data = d4)
summary(mNEO3)

summary(d4$NEOFAC_N)
mNEO4<-lm(NEOFAC_N~BMI*Menstrual_AgeBegan+Race+Age_in_Yrs, data = d4)
summary(mNEO4)

summary(d4$NEOFAC_E)
mNEO5<-lm(NEOFAC_E~BMI*Menstrual_AgeBegan+Race+Age_in_Yrs, data = d4)
summary(mNEO5)
```
...
myvars<-c("Family_ID","Subject","Menstrual_AgeBegan","BMI","ZygosityGT","Age_in_Yrs","Race","Gender","V36","V78","V178","V192","V18")
d5<-twins[myvars]
d5$ZygosityGT<-factor(d5$ZygosityGT)
d5$Race<-factor(d5$Race)
d5<-na.omit(d5)
dd<-d5
#twins0<-subset(d40 , d40$ZygosityGT %in% c('DZ', 'MZ'))
#myvars0<-c("Family_ID","Subject","Menstrual_AgeBegan","BMI","ZygosityGT","Age_in_Yrs","Race","Gender","V36","V78","V178","V192")
#d50<-twins0[myvars0]
#d50$ZygosityGT<-factor(d50$ZygosityGT)
#d50$Race<-factor(d50$Race)
#d50<-na.omit(d50)
d5<-subset(d5, d5$Race != "More than one")
summary(d5$Race)
d5$Race<-factor(d5$Race)


d6 <- fast.reshape(d5, id="Family_ID",varying=c("BMI","Menstrual_AgeBegan","Subject","V36","V78","V178","V192","V18"))
d6<-na.omit(d6)
head(d6)
```
...
mz_V18<-subset(d6, ZygosityGT ==  "MZ"  )[,c("V181","V182")]
p_V18_mz<-scatterdens(mz_V18)
p_V18_mz
...
dz_V18<-subset(d6, ZygosityGT ==  "DZ"  )[,c("V181","V182")]
p_V18_dz<-scatterdens(dz_V18)
p_V18_dz
...
pV18<-ggarrange(p_V18_mz,p_V18_dz + rremove("x.text"),
          labels = c("A", "B"),
          ncol = 2, nrow = 1)
...
summary(as.factor(l2$data$`RaceMore than one`))
2972.483-1309.755
```
##### AIC
...
DZ= "DZ"  , zyg=  "ZygosityGT"  , id="Family_ID", type="aced", missing = T)
summary(as.factor(l3$data$`RaceMore than one`))
summary(l3)
# AIC
AIC(l3.0)
AICc(l3,l3.0)
...
myvars<-c("Family_ID","Subject","Menstrual_AgeBegan","BMI","ZygosityGT","Age_in_Yrs","Race","Gender","V36","V78","V178","V192","V18", "motion")
d5m<-twins[myvars]
d5m$ZygosityGT<-factor(d5m$ZygosityGT)
d5m$Race<-factor(d5m$Race)
d6m <- fast.reshape(d5m, id="Family_ID",varying=c("BMI","Menstrual_AgeBegan","Subject","V36","V78","V178","V192", "V18","motion"))
d6m<-na.omit(d6m)
```
...
```{r}
ddm<-subset(ddm, ddm$Race != "More than one")
summary(ddm$Race)
ddm$Race<-factor(ddm$Race)
levels(ddm$Race)
```
...
```{r}
lX.Xm<-lm(motion~BMI+Age_in_Yrs+Race+Menstrual_AgeBegan, data = d5m)
summary(lX.Xm)
lXm <- twinlm(motion ~ 1+BMI++Age_in_Yrs+Race+Menstrual_AgeBegan, data=ddm,  DZ= "DZ"  , zyg=  "ZygosityGT"  , id="Family_ID", type="aced", missing = T)
summary(lXm)
...
```{r}
l1.0m<-lm(Menstrual_AgeBegan~BMI+Age_in_Yrs+Race+motion, data = d5m)
summary(l1.0m)
l1m <- twinlm(Menstrual_AgeBegan ~ 1+BMI++Age_in_Yrs+Race+motion, data=ddm,  DZ= "DZ"  , zyg=  "ZygosityGT"  , id="Family_ID", type="aced", missing = T)
summary(l1m)
...
```{r}
l2.0m<-lm(V178 ~ BMI*Menstrual_AgeBegan+Age_in_Yrs+Race+motion, data = d5m)
summary(l2.0m)
...
```{r}
l3.0m<-lm(V36 ~ BMI*Menstrual_AgeBegan+Age_in_Yrs+Race+motion, data = d5m)
summary(l2.0m)
...
```{r}
l5.0m<-lm(V18 ~ BMI*Menstrual_AgeBegan+Age_in_Yrs+Race+motion, data = d5m)
summary(l5.0m)
l5m <- twinlm(V18 ~ 1+BMI*Menstrual_AgeBegan+Age_in_Yrs+Race+motion, data=ddm,  DZ= "DZ"  , zyg=  "ZygosityGT"  , id="Family_ID", type="aced", missing = T)
summary(l5m)
...
```{r}
l4.0m<-lm(motion ~ BMI*Menstrual_AgeBegan+Age_in_Yrs+Race, data = d5m)
summary(l4.0m)
l4m <- twinlm(motion ~ 1+BMI+Menstrual_AgeBegan+Age_in_Yrs+Race, data=d5m,  DZ= "DZ"  , zyg=  "ZygosityGT"  , id="Family_ID", type="aced", missing = T)
summary(l4m)
...
plt4m<-ggplot(y4m, aes(x=fac, y=acde.Estimate, color=fac)) +
  ggtitle("Head motion parameters") +
  theme(plot.title = element_text(size = 20, face = "bold"))+
  geom_point(position=position_dodge(), stat="identity",size=n0) +
  geom_errorbar(aes(ymin=acde.2.5., ymax=acde.97.5.),
                size=1,
...
acdem<-ggarrange(plt0m,plt1m,plt2m,plt3m,plt4m + rremove("x.text"),
          labels = c("A", "B","C","D","E"),
          ncol = 2, nrow = 3)
```
...
```{r}
node_df<-read.table("~/Google Drive/HCP_graph/1200/datasets/early_late_nodes_new.csv", header=TRUE, sep=",")
edge_df<-read.table("~/Google Drive/HCP_graph/1200/datasets/early_late_edges_new.csv", header = TRUE, sep=",")
head(node_df)
node_df$module<-as.factor(node_df$module)
names(node_df)
```
...
# Based off doi: 10.3389/neuro.11.037.2009
## Intra-modular connectivity is measured by the normalized within-module degree
## Inter-modular connectivity is measured by the participation coefficient (Pi)
Pi will be close to 1 if the ith node is extensively linked to all other modules in the community and 0 if it is linked exclusively to other nodes in its own module.
...
summary(node_int$group)
node_int6<-subset(node_int, node_int$IC == 'IC6')
node_int6$module<-as.factor(node_int6$module)
summary(node_int6)

node_int3<-subset(node_int, node_int$IC == 'IC3')
node_int3$module<-as.factor(node_int3$module)
summary(node_int3)

node_int12<-subset(node_int, node_int$IC == 'IC12')
node_int12$module<-as.factor(node_int12$module)
summary(node_int12)


node_int13<-subset(node_int, node_int$IC == 'IC13')
node_int13$module<-as.factor(node_int13$module)

node_int2<-subset(node_int, node_int$IC == 'IC2')
...
levels(node_int6$module)
freqin6.0 <- xtabs(~early_late+module, data=node_int6)
ftable(freqin6.0) # print table
summary(freqin6.0)

node_int6$node <- droplevels(node_int6$node)

freqin6.1 <- xtabs(~early_late+module+node, data=node_int6)
ftable(freqin6.1) # print table
summary(freqin6.1)
```
# Summary
No difference in the frequencies between modules in the late vs. early
No difference in the composition of the nodes in the modules
```{r}
...
pairs(means.el, adjust = "bon")
```
# Summary
No difference in CC

```{r}
describeBy(node_int6$cc, node_int6$early_late)
cc6p0 <- ggplot(node_int6, aes(x=early_late, y=cc, color=early_late, fill=early_late)) +
...
```{r}
PC6<-lm(PC~module*early_late, data=node_int6)
summary(PC6)

means.mod <- lsmeans(PC6, specs = "module")
means.el  <- lsmeans(PC6, specs = "early_late")
summary(means.el)
means.int <- lsmeans(PC6, specs = c("module", "early_late"))
means.int
pairs(means.int, adjust = "bon")
# pairs(means.el, adjust = "bon")
```
```{r}
# Basic violin plot
PC6p <- ggplot(node_int6, aes(x=module, y=PC, color=module, fill=module)) +
  geom_violin()+ facet_grid( . ~ early_late)+theme_classic()
PC6p
```

```{r}
btw6<-lm(btw~module*early_late, data=node_int6)

means.btwmod <- lsmeans(btw6, specs = "module")
means.btwel  <- lsmeans(btw6, specs = "early_late")
means.btwint <- lsmeans(btw6, specs = c("module", "early_late"))
means.btwint
pairs(means.btwint, adjust = "bon")
pairs(means.btwel, adjust = "bon")
```
...
# IC 3
```{r}
levels(node_int3$module)
levels(node_int3$early_late)
freqin3 <- xtabs(~early_late+module, data=node_int3)
ftable(freqin3) # print table

node_int3<-node_int3[!(node_int3$module=="5" | node_int3$module=="6"  | node_int3$module=="7" | node_int3$module=="8" | node_int3$module=="9"),]
node_int3$module<-factor(node_int3$module)
levels(node_int3$module)



freqin3.0 <- xtabs(~early_late+module, data=node_int3)
ftable(freqin3.0) # print table
summary(freqin3.0)

node_int3$node <- droplevels(node_int3$node)

freqin3.1 <- xtabs(~early_late+module+node, data=node_int3)
ftable(freqin3.1) # print table
summary(freqin3.1)
```
...
```{r}
cc3<-lm(cc~module*early_late, data=node_int3)
summary(cc3)
means.mod3 <- lsmeans(cc3, specs = "module")
means.el3  <- lsmeans(cc3, specs = "early_late")
means.int3 <- lsmeans(cc3, specs = c("module", "early_late"))
means.int3
pairs(means.int3, adjust = "bon")
pairs(means.el3, adjust = "bon")
```
...
```{r}
# Basic violin plot
cc3p <- ggplot(node_int3, aes(x=early_late, y=cc, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
cc3p
```

```{r}
PC3<-lm(PC~module*early_late, data=node_int3)
summary(PC3)

means.mod <- lsmeans(PC3, specs = "module")
means.el  <- lsmeans(PC3, specs = "early_late")
summary(means.el)
means.int <- lsmeans(PC3, specs = c("module", "early_late"))
means.int
pairs(means.int, adjust = "bon")
# pairs(means.el, adjust = "bon")
```
...
```{r}
btw3<-lm(btw~module*early_late, data=node_int3)
means.mod3btw <- lsmeans(btw3, specs = "module")
means.el3btw  <- lsmeans(btw3, specs = "early_late")
means.int3btw <- lsmeans(btw3, specs = c("module", "early_late"))
means.el3btw
pairs(means.el3btw, adjust = "bon")
```
#Summary
No differences
...
# IC 12
```{r}
levels(node_int12$module)
levels(node_int12$early_late)

freqin12 <- xtabs(~early_late+module, data=node_int12)
ftable(freqin12) # print table

node_int12<-node_int12[!(node_int12$module=="5" | node_int12$module=="6" | node_int12$module=="7" | node_int12$module=="8" | node_int12$module=="9"),]
node_int12$module<-factor(node_int12$module)

levels(node_int12$module)
freqin12.0 <- xtabs(~early_late+module, data=node_int12)
ftable(freqin12.0) # print table
summary(freqin12.0)

node_int12$node <- droplevels(node_int12$node)

freqin12.1 <- xtabs(~early_late+module+node, data=node_int12)
ftable(freqin12.1) # print table
summary(freqin12.1)
```
# Summary
No difference in the frequencies between modules in the late vs. early
No difference in the composition of the nodes in the modules
```{r}
cc12<-lm(cc~module*early_late, data=node_int12)
summary(cc12)

means.mod12 <- lsmeans(cc12, specs = "module")
means.el12  <- lsmeans(cc12, specs = "early_late")
summary(means.el12)

means.int12 <- lsmeans(cc12, specs = c("module", "early_late"))
means.int12

pairs(means.int12, adjust = "bon")
pairs(means.el12, adjust = "bon")
```
# Summary
No difference in CC

```{r}
PC12<-lm(PC~module*early_late, data=node_int12)
summary(PC12)

means.mod <- lsmeans(PC12, specs = "module")
means.el  <- lsmeans(PC12, specs = "early_late")
summary(means.el)
means.int <- lsmeans(PC12, specs = c("module", "early_late"))
means.int
pairs(means.int, adjust = "bon")
# pairs(means.el, adjust = "bon")
```
```{r}
# Basic violin plot
PC3p <- ggplot(node_int3, aes(x=early_late, y=PC, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
PC3p
```

```{r}
btw12<-lm(btw~module*early_late, data=node_int12)

means.btwmod12 <- lsmeans(btw12, specs = "module")
means.btwel12  <- lsmeans(btw12, specs = "early_late")
means.btwint12 <- lsmeans(btw12, specs = c("module", "early_late"))
means.btwint12
pairs(means.btwint12, adjust = "bon")
pairs(means.btwel12, adjust = "bon")
```
# Summary
No differences in btw centrality


# IC 13
```{r}
levels(node_int13$module)
levels(node_int13$early_late)

freqin13 <- xtabs(~early_late+module, data=node_int13)
ftable(freqin13) # print table

node_int13<-node_int13[!(node_int13$module=="6" | node_int13$module=="7" | node_int13$module=="8" | node_int13$module=="9"),]
node_int13$module<-factor(node_int13$module)

levels(node_int13$module)
freqin13.0 <- xtabs(~early_late+module, data=node_int13)
ftable(freqin13.0) # print table
summary(freqin13.0)

node_int13$node <- droplevels(node_int13$node)

freqin13.1 <- xtabs(~early_late+module+node, data=node_int13)
ftable(freqin13.1) # print table
summary(freqin13.1)
```
# Summary
No difference in the frequencies between modules in the late vs. early
No difference in the composition of the nodes in the modules
```{r}
cc13<-lm(cc~module*early_late, data=node_int13)
summary(cc13)

means.mod13 <- lsmeans(cc13, specs = "module")
means.el13  <- lsmeans(cc13, specs = "early_late")
summary(means.el13)

means.int13 <- lsmeans(cc13, specs = c("module", "early_late"))
means.int13

pairs(means.int13, adjust = "bon")
pairs(means.el13, adjust = "bon")
```
# Summary
No difference in CC
...
```{r}
btw13<-lm(btw~module*early_late, data=node_int13)

means.btwmod13 <- lsmeans(btw13, specs = "module")
means.btwel13  <- lsmeans(btw13, specs = "early_late")
means.btwint13 <- lsmeans(btw13, specs = c("module", "early_late"))
means.btwint13
pairs(means.btwint13, adjust = "bon")
pairs(means.btwel13, adjust = "bon")
```
...
# IC 2
```{r}
levels(node_int2$module)
levels(node_int2$early_late)

freqin2 <- xtabs(~early_late+module, data=node_int2)
ftable(freqin2) # print table

node_int2<-node_int2[!(node_int2$module=="8" | node_int2$module=="9"),]
node_int2$module<-factor(node_int2$module)

levels(node_int2$module)
freqin2.0 <- xtabs(~early_late+module, data=node_int2)
ftable(freqin2.0) # print table
summary(freqin2.0)

node_int2$node <- droplevels(node_int2$node)

freqin2.1 <- xtabs(~early_late+module+node, data=node_int2)
ftable(freqin2.1) # print table
summary(freqin2.1)
```
# Summary
No difference in the frequencies between modules in the late vs. early
No difference in the composition of the nodes in the modules
```{r}
cc2<-lm(cc~module*early_late, data=node_int2)
summary(cc2)

means.mod2 <- lsmeans(cc2, specs = "module")
means.el2  <- lsmeans(cc2, specs = "early_late")
summary(means.el2)

means.int2 <- lsmeans(cc2, specs = c("module", "early_late"))
means.int2

pairs(means.int2, adjust = "bon")
pairs(means.el2, adjust = "bon")
```
# Summary
Difference in clustering coefficient between early and late within module 0

```{r}
# Basic violin plot
cc2p <- ggplot(node_int2, aes(x=module, y=cc, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
cc2p
```
...
```{r}
btw2<-lm(btw~module*early_late, data=node_int2)

means.btwmod2 <- lsmeans(btw2, specs = "module")
means.btwel2  <- lsmeans(btw2, specs = "early_late")
means.btwint2 <- lsmeans(btw2, specs = c("module", "early_late"))
means.btwint2
pairs(means.btwint2, adjust = "bon")
pairs(means.btwel2, adjust = "bon")
```


```{r}
node_int2_mod0<-subset(node_int2, node_int2$module == '0')
node_int2_mod0$module<-factor(node_int2_mod0$module)
summary(node_int2_mod0$module)

freqin2.0 <- xtabs(~early_late, data=node_int2_mod0)
ftable(freqin2.0) # print table


node_int2_mod0$node <- droplevels(node_int2_mod0$node)

freqin2.01 <- xtabs(~early_late+node, data=node_int2_mod0)
ftable(freqin2.01) # print table
summary(freqin2.01)
```

```{r}
describeBy(node_int2_mod0$cc, node_int2_mod0$early_late)
cc2n<-lm(cc~node*early_late, data=node_int2_mod0)
summary(cc2n)

means.mod2 <- lsmeans(cc2n, specs = "node")
means.el2  <- lsmeans(cc2n, specs = "early_late")
summary(means.el2)

means.int2 <- lsmeans(cc2, specs = c("node", "early_late"))
means.int2
...
```{r}
p <- ggplot(node_int2_mod0, aes(x =early_late, y = node))
p+geom_point( aes(size=cc,colour=early_late))+theme(panel.background=element_blank())
```

# Edge analysis
```{r}
edge_int<-subset(edge_df, edge_df$group == 'int')
summary(edge_int)
edge_int$edgeID<- with(edge_int, paste0(x,y))

edge_int6<-subset(edge_int, edge_int$IC == 'IC6')
summary(edge_int6)

edge_int3<-subset(edge_int, edge_int$IC == 'IC3')
summary(edge_int3)

edge_int12<-subset(edge_int, edge_int$IC == 'IC12')
summary(edge_int12)


edge_int13<-subset(edge_int, edge_int$IC == 'IC13')

edge_int2<-subset(edge_int, edge_int$IC == 'IC2')
```

```{r}
eAll<-lm(edge_weight~early_late*IC, data=edge_int)
summary(eAll)

means.eAll  <- lsmeans(eAll, specs = c("early_late","IC"))
summary(means.eAll)
pairs(means.eAll, adjust = "bon")
means.eAll@V

eEarly<-lm(edge_weight~IC, data=subset(edge_int,edge_int$early_late=='early'))
summary(eEarly)
means.eEarly  <- lsmeans(eEarly, specs = "IC")
pairs(means.eEarly, adjust = "bon")

eLate<-lm(edge_weight~IC, data=subset(edge_int,edge_int$early_late=='late'))
summary(eLate)
means.eLate  <- lsmeans(eLate, specs = "IC")
summary(means.eLate)
pairs(means.eLate, adjust = "bon")

eAllp <- ggplot(edge_int, aes(x=IC, y=edge_weight, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
eAllp
```
...
# IC6
```{r}
edge_int6$edgeID<-factor(edge_int6$edgeID)
freqin6e <- xtabs(~early_late+edgeID, data=edge_int6)
ftable(freqin6e) # print table

summary(edge_int6)

edge_int6$edgeID <- droplevels(edge_int6$edgeID)
freqin6e <- xtabs(~early_late+edgeID, data=edge_int6)
ftable(freqin6e) # print table
summary(freqin6e)

summary(edge_int6$edge_weight)
edge_int6_third<-subset(edge_int6,edge_int6$edge_weight >= 0.65)
pe6 <- ggplot(edge_int6_third, aes(x =early_late, y = edgeID))
pe6+geom_point( aes(size=edge_weight,colour=early_late))+theme(panel.background=element_blank())

```

```{r}
e6<-lm(edge_weight~early_late, data=edge_int6)
summary(e6)

means.e6  <- lsmeans(e6, specs = "early_late")
summary(means.e6)

pairs(means.e6, adjust = "bon")

# Basic violin plot
e6p <- ggplot(edge_int6, aes(x=early_late, y=edge_weight, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
e6p
```
...
#IC3
```{r}
edge_int3$edgeID<-factor(edge_int3$edgeID)
freqin3e <- xtabs(~early_late+edgeID, data=edge_int3)
ftable(freqin3e) # print table

edge_int3$edgeID <- droplevels(edge_int3$edgeID)
freqin3e <- xtabs(~early_late+edgeID, data=edge_int3)
ftable(freqin3e) # print table
summary(freqin3e)

summary(edge_int3$edge_weight)
edge_int3_third<-subset(edge_int3,edge_int3$edge_weight >= 0.65)
pe3 <- ggplot(edge_int3_third, aes(x =early_late, y = edgeID))
pe3+geom_point( aes(size=edge_weight,colour=early_late))+theme(panel.background=element_blank())
```

```{r}
e3<-lm(edge_weight~early_late, data=edge_int3)
summary(e3)

means.e3  <- lsmeans(e3, specs = "early_late")
summary(means.e3)

pairs(means.e3, adjust = "bon")

# Basic violin plot
e3p <- ggplot(edge_int3, aes(x=early_late, y=edge_weight, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
e3p
```
...
# IC12
```{r}
edge_int12$edgeID<-factor(edge_int12$edgeID)
edge_int12$edgeID <- droplevels(edge_int12$edgeID)

freqin12e <- xtabs(~early_late+edgeID, data=edge_int12)
ftable(freqin12e) # print table
summary(freqin12e)

summary(edge_int12$edge_weight)
edge_int12_third<-subset(edge_int12,edge_int12$edge_weight >= 0.65)
pe12 <- ggplot(edge_int12_third, aes(x =early_late, y = edgeID))
pe12+geom_point( aes(size=edge_weight,colour=early_late))+theme(panel.background=element_blank())
```

```{r}
e12<-lm(edge_weight~early_late, data=edge_int12)
summary(e12)

means.e12  <- lsmeans(e12, specs = "early_late")
summary(means.e12)

pairs(means.e12, adjust = "bon")

# Basic violin plot
e12p <- ggplot(edge_int12, aes(x=early_late, y=edge_weight, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
e12p
```
```{r}
edge_int13$edgeID<-factor(edge_int13$edgeID)
freqin13e <- xtabs(~early_late+edgeID, data=edge_int13)
ftable(freqin13e) # print table

summary(edge_int13)

edge_int13$edgeID <- droplevels(edge_int13$edgeID)
freqin13e <- xtabs(~early_late+edgeID, data=edge_int13)
ftable(freqin13e) # print table
summary(freqin13e)

summary(edge_int13$edge_weight)
edge_int13_third<-subset(edge_int13,edge_int13$edge_weight >= 0.85)
pe13 <- ggplot(edge_int13_third, aes(x =early_late, y = edgeID))
pe13+geom_point( aes(size=edge_weight,colour=early_late))+theme(panel.background=element_blank())
```

```{r}
e13<-lm(edge_weight~early_late, data=edge_int13)
summary(e13)

means.e13  <- lsmeans(e13, specs = "early_late")
summary(means.e13)

pairs(means.e13, adjust = "bon")

# Basic violin plot
e13p <- ggplot(edge_int13, aes(x=early_late, y=edge_weight, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
e13p
```
```{r}
edge_int2$edgeID<-factor(edge_int2$edgeID)

edge_int2$edgeID <- droplevels(edge_int2$edgeID)
freqin2e <- xtabs(~early_late+edgeID, data=edge_int2)
ftable(freqin2e) # print table
summary(freqin2e)

summary(edge_int2$edge_weight)
edge_int2_third<-subset(edge_int2,edge_int2$edge_weight >= 0.7)
pe2 <- ggplot(edge_int2_third, aes(x =early_late, y = edgeID))
pe2+geom_point( aes(size=edge_weight,colour=early_late))+theme(panel.background=element_blank())
```

```{r}
e2<-lm(edge_weight~early_late, data=edge_int2)

means.e2  <- lsmeans(e2, specs = "early_late")
summary(means.e2)

pairs(means.e2, adjust = "bon")

# Basic violin plot
e2p <- ggplot(edge_int2, aes(x=early_late, y=edge_weight, color=early_late, fill=early_late)) +
  geom_violin()+ theme_classic()
e2p
```
...
### RM Corr
```{r}
cortest(e23,l23,n1=87,n2 =125, fisher = TRUE,cor=TRUE)
```
...
```{r}
node_int$hubbness[node_int$zDegree >= 2.5]<-"hub"
node_int$hubbness[node_int$zDegree < 2.5]<-"non_hub"
summary(node_int$hubbness)
```
# No hubs

For hubs, 0  <   Pi  <  0.30    provincial hub , 
0.30  <   Pi  <  0.75   connector hub , and 
0.75  <   Pi  <  1.0 is a  kinless hub .

For non-hubs, if a given node  has value 
...
```{r}
node_int$node_type[node_int$PC > 0 & node_int$PC <= 0.05 ]<-"ultra"
node_int$node_type[node_int$PC > 0.05 & node_int$PC <= 0.62 ]<-"peripheral"
node_int$node_type[node_int$PC > 0.62 & node_int$PC <= 0.8 ]<-"connector"
node_int$node_type[node_int$PC > 0.8 & node_int$PC <= 1 ]<-"kinless"
summary(as.factor(node_int$node_type))
```


```{r}
node_int$module<-as.factor(node_int$module)
node_int$group<-as.factor(node_int$group)
node_int$early_late<-as.factor(node_int$early_late)
node_int$node<-as.factor(node_int$node)
node_int$node_type<-as.factor(node_int$node_type)
  
summary(node_int$IC)
IC2<-subset(node_int, node_int$IC == "IC2")
IC3<-subset(node_int, node_int$IC == "IC3")
IC6<-subset(node_int, node_int$IC == "IC6")
IC12<-subset(node_int, node_int$IC == "IC12")
IC13<-subset(node_int, node_int$IC == "IC3")
```


```{r}
summary(IC2$module)
IC2_early<-subset(IC2, IC2$early_late == "early")
describeBy(IC2_early, IC2_early$module)
describeBy(IC2, IC2$group=="early")
mytable <- xtabs(~node_type+module, data=IC2_early)
mytable
...
```{r}
summary(IC2$module)
IC2_late<-subset(IC2, IC2$early_late == "late")
describeBy(IC2_late, IC2_late$module)
mytable <- xtabs(~node_type+module, data=IC2_late)
mytable
margin.table(mytable, 2)
```
```{r}
summary(IC3$module)
IC3_early<-subset(IC3, IC3$early_late == "early")
describeBy(IC3_early, IC3_early$module)
mytable <- xtabs(~node_type+module, data=IC3_early)
mytable
```
```{r}
summary(IC3_late$module)
IC3_late<-subset(IC3, IC3$early_late == "late")
head(IC3_late)
describeBy(IC3_late, IC3_late$module)
mytable <- xtabs(~node_type+module, data=IC3_late)
mytable
```
```{r}
summary(IC6_early$module)
IC6_early<-subset(IC6, IC6$early_late == "early")
head(IC6_early)
describeBy(IC6_early, IC6_early$module)
mytable <- xtabs(~node_type+module, data=IC6_early)
mytable
```


```{r}
summary(IC6_late$module)
IC6_late<-subset(IC6, IC6$early_late == "late")
head(IC6_late)
describeBy(IC6_late, IC6_late$module)
mytable <- xtabs(~node_type+module, data=IC6_late)
mytable
```
```{r}
summary(IC12_early$module)
IC12_early<-subset(IC12, IC12$early_late == "early")
head(IC12_early)
describeBy(IC12_early, IC12_early$module)
mytable <- xtabs(~node_type+module, data=IC12_early)
mytable
```


```{r}
summary(IC12_late$module)
IC12_late<-subset(IC12, IC12$early_late == "late")
head(IC12_late)
describeBy(IC12_late, IC12_late$module)
mytable <- xtabs(~node_type+module, data=IC12_late)
mytable
```

```{r}
IC13_early<-subset(IC13, IC13$early_late == "early")
summary(IC13_early$module)
head(IC13_early)
describeBy(IC13_early, IC13_early$module)
mytable <- xtabs(~node_type+module, data=IC13_early)
mytable
```

```{r}
IC13_late<-subset(IC13, IC13$early_late == "late")
summary(IC13_late$module)
head(IC13_late)
describeBy(IC13_late, IC13_late$module)
mytable <- xtabs(~node_type+module, data=IC13_late)
mytable
```

```{r}
conns<-subset(node_int, node_int$node_type == "connector")
conns$module<-factor(conns$module)
mytable <- xtabs(~early_late+module, data=conns)
summary(mytable)
mytable
subset(conns, conns$early_late == "early" & conns$module == 3)
```
